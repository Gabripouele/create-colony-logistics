package com.createcolonylogistics.client;

import com.createcolonylogistics.CreateColonyLogistics;
import com.createcolonylogistics.registry.CCLMenus;
import net.neoforged.api.distmarker.Dist;
import net.neoforged.bus.api.SubscribeEvent;
import net.neoforged.fml.common.EventBusSubscriber;
import net.neoforged.neoforge.client.event.RegisterMenuScreensEvent;

@EventBusSubscriber(modid = CreateColonyLogistics.MOD_ID, bus = EventBusSubscriber.Bus.MOD, value = Dist.CLIENT)
public final class CCLClientEvents {
    private CCLClientEvents() {
    }

    @SubscribeEvent
    public static void registerMenuScreens(RegisterMenuScreensEvent event) {
        event.register(CCLMenus.SMART_CLIPBOARD.get(), SmartClipboardScreen::new);
    }
}
